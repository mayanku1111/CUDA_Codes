...
__global__ void matrixMul(const int *a, const int *b, int *c)
{
    ...
    for (int i = 0; i < N; i += blockDim.x)
    {
        ...
        // s_a[threadIdx.y * blockDim.x + threadIdx.x] converts 2d representation to 1D
        ...
        // Every Thread Has Its Own tmp, it is a local variable it lives in each thread's private registers
        // Inside one tile computation the k-dimension tiles are sequential
...
// #CUDA follows this rule:

// #Programmer defines grid and blocks.GPU runtime schedules blocks onto SMs automatically.

#Comment

// A block always stays on the same SM for its entire lifetime.

// So this loop:

// for (int i = 0; i < N; i += blockDim.x)

// runs entirely on the same SM.
...
// The block never migrates to another SM.